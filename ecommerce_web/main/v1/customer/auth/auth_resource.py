...
# Customer Registration Resource
class CustomerRegistrationResource(Resource):
    def post(self):
        ...
        new_user = User(
            ...
            role="2",  # Customer role as per requirement
...
# Customer Login Resource (Without Refresh Token)
class CustomerLoginResource(Resource):
    def post(self):
        ...
        if not user or not user.check_password(data.get("password")) or user.role != "2":
